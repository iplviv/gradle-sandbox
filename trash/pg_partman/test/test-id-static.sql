-- ########## ID STATIC TESTS ##########
-- Other tests: Turn off jobmon logging, additional constraints single column, larger than necessary p_batch_count to partition_data_id()
...
SELECT create_parent('partman_test.id_static_table', 'col1', 'id-static', '10', '{"col3"}', p_jobmon := false);
...
SELECT results_eq('SELECT partition_data_id(''partman_test.id_static_table'', p_batch_count := 5)::int', ARRAY[9], 'Check that partitioning function returns correct count of rows moved');
...
SELECT hasnt_table('partman_test', 'id_static_table_p100', 'Check id_static_table_p100 doesn''t exists yet');
